...
'''
Class for that will request user to upload a file in the specified input template, extract data, format data, 
transform data according to the specified output template, and return to user to download
'''
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            if self.path == '/contacts-allassured-merlin':
                self.send_response(200)
                self.send_header('Content-type', 'text/plain')
                self.end_headers()
                self.wfile.write('Preparing contacts from AllAssured to Merlin'.encode('utf-8'))
